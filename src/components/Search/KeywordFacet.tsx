...
export function KeywordFacet(props: {
  ...
  onChangeKeywordFacet: (facetName: string, facetOptionName: string, selected: boolean) => void;
}) {
  ...
  return <div className="w-full max-w-[450px]">
    ...
    {Object.entries(props.facet).map(
        ([facetOptionName, facetOption]) => {
          const isSelected = !!props.selectedFacets[props.facetName]?.includes(facetOptionName);
              ...
              >
                <div className="flex flex-row items-center">
                  <input
                      ...
                      onChange={() =>
                          props.onChangeKeywordFacet(props.facetName, facetOptionName, !isSelected)
                      }
                      checked={isSelected}
                  />
                  <label htmlFor={facetOptionKey}>
                    {/^[a-z]/.test(facetOptionName)
                        ? facetOptionName.charAt(0).toUpperCase() + facetOptionName.slice(1)
