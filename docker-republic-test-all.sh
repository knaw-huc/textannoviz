...
docker build -t textannoviz-${PROJECT}-test-frontend:${VERSION} --platform=linux/amd64 -f deploy/Dockerfile-deploy .

docker tag textannoviz-${PROJECT}-test-frontend:${VERSION} ${DOCKER_DOMAIN}/textannoviz-${PROJECT}-test-frontend:${VERSION}

docker push ${DOCKER_DOMAIN}/textannoviz-${PROJECT}-test-frontend:${VERSION}
